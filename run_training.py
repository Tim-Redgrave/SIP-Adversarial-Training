...
dataloader = CUB_data_loader.create_data_loader(data_split_path=args.data_split_path, data_root_path=args.datasetPath, map_location=args.heatmaps,network=args.network,batch_size=args.batchSize)
...
with open(os.path.join(output_path,'params.json'), 'w') as out:
    hyper = vars(args)
    json.dump(hyper, out)
...
for epoch in range(args.nEpochs):
    ...
    with torch.set_grad_enabled(True):
        for batch_idx, (data, cls, imageName, hmap) in enumerate(dataloader["train"]):
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, (data, cls, imageName, hmap) in enumerate(dataloader["test"]):
            batch_size = len(data)
            data = data.to(device)
            cls = cls.to(device)
